...
5. **Concurrency.** There is no shared log file to guard: each observation
   ...
   another session's entry. Before changing the _status_ of an existing
...
Libraries accumulate _families_ — several skills implementing one
...
_absence_ of the judgement visible to a review or a drift audit.
